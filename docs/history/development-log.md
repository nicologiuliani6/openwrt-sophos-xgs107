> **Historical record** of the bring-up, kept as it was written (the
> "session" notes, dead ends included). For current instructions read the
> documents in [`docs/`](..). Script names in this log are the old ones:
>
> | Old | Now |
> |---|---|
> | `scripts/40-console-send.py`, `91-console-log.py`, `90-x86-grub.py`, `50-dump-receiver.py`, `97-console-put.py` | `tools/console-send.py`, `console-log.py`, `x86-grub.py`, `dump-receiver.py`, `console-put.py` |
> | `scripts/80-mk-emmc-slot.sh` | `build/mk-emmc-slot.sh` |
> | `scripts/81-uboot-env-openwrt.txt`, `82-install-openwrt.sh`, `95-x86-install.sh`, `96-x86-mbr.py` | `install/uboot-env.txt`, `install-npu.sh`, `install-x86.sh`, `x86-mbr.py` |
> | `scripts/30-`, `60-`, `61-`, `70-` (NPU header console, kexec RAM boots, telnet) | removed; they depended on a bench initramfs and are in the git history |
> | `driver/`, `scripts/00-`, `05-`, `10-`, `20-` (Prestera track) | removed, obsolete: see [prestera-notes.md](prestera-notes.md) |
...
[hardware-architecture.md](../hardware.md). Headline findings:
...
- **Every NPU dump was verified** against a `sha256sum` run on the NPU
  ...
  [dumps-manifest.md](../reference/dumps-manifest.md).
...
- Procedure: [openwrt-install.md](../install.md).
...
3. U-Boot env applied with `fw_setenv -s`:
   ...
   [openwrt-install.md](../install.md). Read back OK.
...
[x86-openwrt.md](../architecture.md). Outline and lessons:
...
Details of the link in [x86-npu-link.md](../npu-x86-link.md).
